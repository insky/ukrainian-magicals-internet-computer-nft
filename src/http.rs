...
fn http_request() {
  ...
  ledger::with(|ledger| {
    ...
    let mut code = 200;
    if root == "" {
      body = format!("Total NFTs: {}", ledger.tokens.len())
        .into_bytes()
        .into();
    } else {
      if let Ok(num) = root.parse::<Nat>() {
        // /:something
        if let Some(nft) = ledger.tokens.get(&num) {
          // /:nft
          let img = path.next().unwrap_or_else(|| "".into());
          if img == "" {
            // /:nft/
            let part = nft.properties.get(0);
            if let Some((key, value)) = part {
              ...
              // }
              body = format!("{}: {:?}", key, value).into_bytes().into();
            } else {
            ...
            //   // /:nft/:number
            //   if let Some(part) = nft.properties.get(num) {
            //     // /:nft/:id
            //     body = part.data.as_slice().into();
            //     if let Some(MetadataVal::TextContent(mime)) = part.key_val_data.get("contentType") {
            //       headers.insert("Content-Type", mime.as_str().into());
